...
void test_max_sampling_rate_imu_hx711()
{
    Serial.println("\n[TEST] Max sampling rate IMU + HX711");
    ...
    TEST_ASSERT_TRUE_MESSAGE(scale.wait_ready_timeout(3000), "HX711 not ready at start.");
    ...
    Serial.println("Collecting synchronized samples with minimum interval...");
    for (int i = 0; i < sampleCount; i++)
    {
        while (!scale.is_ready())
        {
            // Busy wait to keep minimum time between fused samples.
        }
    ...
    Serial.printf("Last IMU A=%d,%d,%d G=%d,%d,%d | HX=%ld\n",
    ...
    TEST_ASSERT_TRUE_MESSAGE(avgHz > 0.1f, "Average sample rate too low.");
}
...
void setup()
{
    ...
    Serial.println("\n=== petBionics high-rate sampling test ===");
    Serial.printf("Pins SPI SCK=%d MISO=%d MOSI=%d CS_IMU=%d | HX DT=%d SCK=%d\n",
    ...
    UNITY_BEGIN();
    RUN_TEST(test_max_sampling_rate_imu_hx711);
    UNITY_END();
}
