...
void writeCSVLine(const char* line){ 
  if(csvFile){ 
    ...
    csvFile.flush(); 
    if(csvFile.size()>=MAX_FILE_SIZE) openNewCSV(); // rotate CSV independently
  } 
}
...
void writeTXTLine(const char* timestamp,float weight,float roll,float pitch,float yaw,float ax,float ay,float az,float gx,float gy,float gz,float vbat,int pct){
  ...
  txtFile.flush();
  if(txtFile.size()>=MAX_FILE_SIZE) openNewTXT(); // rotate TXT independently
}
...
void loop(){
  ...
  if(loggingStartTime == 0) loggingStartTime = now;
  ...
  if(now - loggingStartTime >= LOG_DURATION_MS){
    Serial.println("Logging period complete. Going to deep sleep...");
    if(csvFile){ csvFile.close(); }
    if(txtFile){ txtFile.close(); }
    delay(100); // ensure SD flush
