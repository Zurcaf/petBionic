...
// ================= TIME MANAGEMENT =================
uint32_t startMillis = 0;   // millis() at NTP sync
time_t baseEpoch = 0;       // epoch time at sync
...
void wifiSetup(){
  ...
  if(WiFi.status() == WL_CONNECTED){
    ...
    baseEpoch = timeClient.getEpochTime();  // Save epoch at startup
    startMillis = millis();                  // Save millis reference
...
String createDailyFolder(){
  time_t nowEpoch = baseEpoch + (millis()-startMillis)/1000;
...
String generateTimeFile(String ext){
  time_t nowEpoch = baseEpoch + (millis()-startMillis)/1000;
  uint32_t msPart = (millis()-startMillis)%1000;
  struct tm *t = localtime(&nowEpoch);
  char fileName[50];
  sprintf(fileName,"%s/%02d-%02d-%02d.%03d.%s",
          currentFolder.c_str(), t->tm_hour, t->tm_min, t->tm_sec, msPart, ext.c_str());
...
void setup(){
  ...
  wifiSetup();           // sync time and disable WiFi
  sdInit();              // init SD
...
void loop(){
  ...
  time_t nowEpoch = baseEpoch + elapsedMillis / 1000;
  uint16_t msPart = elapsedMillis % 1000;
  struct tm *t = localtime(&nowEpoch);
  char timestamp[20]; 
  sprintf(timestamp, "%02d:%02d:%02d.%03d", t->tm_hour, t->tm_min, t->tm_sec, msPart);
